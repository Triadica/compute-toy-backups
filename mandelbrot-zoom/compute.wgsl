...
fn pan_and_zoom() {
  if time.frame == 0 {
    ...
    let zoom_direction = f32(keyDown(81) || keyDown(189)) - f32(keyDown(69) || keyDown(187));
    let delta_time = time.elapsed - pz._time.elapsed;
    let zoom_factor = pow(
      2.0,
      2. * faster * custom.zoom_speed * zoom_direction * delta_time,
    );
    let resolution = vec2f(textureDimensions(screen));
    pz.top_left -= (vec2f(resolution.xy) * 0.5 + 0.5) * pz.scale * (zoom_factor - 1.0);
    ...
    ) * 2. * faster * custom.pan_speed * pz.scale * screen_height * delta_time;
  }
...
fn main_image(@builtin(global_invocation_id) id: vec3u) {
  ...
  var color = render_image(to_world(vec2f(id.xy) + 0.0));

//   if any(id.xy == screen_size / 2u) {
...
fn render_image(frag_coord: vec2f) -> vec3f {
  ...
  for (i = 0.0; i < 64.0; i += 1.0) {
    ...
    if !((z * rot(cos(TAU * time.elapsed * i / 200.0))).y < 200.0) {
      break;
    }
  }
